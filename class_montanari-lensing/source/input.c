...
int input_init(
  ...
  if (ppt->has_cl_number_count == _TRUE_) {
    ...
    if (flag1 == _TRUE_) {
      ...
      if (flag1 == _TRUE_){
        ...
        ptr->err_El = param1;
        pop->has_el_number_count = _TRUE_;
      }
  ...
  class_call(parser_read_double(pfc,"MG_beta2",&param1,&flag1,errmsg),
             errmsg,
             errmsg);
  class_test(((flag1 == _TRUE_) && ppt->gauge == synchronous),
             errmsg,
             "Modified Gravity is not implemented with synchronous gauge yet");
  if (flag1 == _TRUE_)
    ppt->mg_beta2 = param1;
    ppt->has_mg = _TRUE_;
    ppt->gauge = newtonian;
  ...
  if ((ppt->has_cl_number_count == _TRUE_) || (ppt->has_cl_lensing_potential == _TRUE_)) {
    ...
    if ((flag1 == _TRUE_) && (int1>0)) {
      ...
      free(pointer1);
      /* first set all widths and bias to default; correct eventually later */
      for (i=1; i<int1; i++) {
      ...
      if ((flag1 == _TRUE_) && (int1>0)) {
        ...
        if (int1==1) {
          ...
          class_stop(errmsg,
                     "In input for selection function, you asked for %d bin centers and %d bin galaxy bias; number of bins unclear; you should pass either one bin bias (common to all bins) or %d bin bias",
                     ppt->selection_num,int1,ppt->selection_num);
        }
        free(pointer1);
      }
    ...
    if ((flag1 == _TRUE_)) {
      if ((strstr(string1,"analytic_euclid") != NULL))
        ptr->has_nz_evo_analytic_euclid = _TRUE_;
      else if ((strstr(string1,"analytic_ska") != NULL))
        ptr->has_nz_evo_analytic_ska = _TRUE_;
      else{
...
int input_default_params(
  ...
  ptr->has_nz_analytic_euclid = _FALSE_;
