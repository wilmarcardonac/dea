...
int nonlinear_init(
  ...
  double *pk_l;
  double *pk_nl;
  /* for calling background_at_tau */
  int last_index;
  double * pvecback = NULL;
  /* to compute k_nl by extrapolation of halofit results */
  int last_index_tau_nl;
  double last_a_nl;
  ...
  /** (b) for HALOFIT non-linear spectrum */
    ...
    for (index_tau = pnl->tau_size-1; index_tau>=0; index_tau--) {
      ...
      if (nonlinear_halofit(ppr,
        ...
        last_index_tau_nl = index_tau;
        last_a_nl = pvecback[pba->index_bg_a];
        ...
        /* When k_nl is not computed by nonlinear_halofit(), use
           k_nl(a) \propto a^(-2/(3+n_s))
         */
        pnl->k_nl[index_tau] = pnl->k_nl[last_index_tau_nl] /
                               pow( pvecback[pba->index_bg_a]/last_a_nl , 2./(3.+ppm->n_s) );
      }
