  ...
  for (index_q=0; index_q < ptr->q_size; index_q++) {
    ...
    if (ppt->has_cl_number_count == _TRUE_) {
      ...
      for (index_d1=0; index_d1<psp->d_size; index_d1++) {
        ...
        /* if error spectra El's are required, compute the absolute value of the transfers */
        if (ptr->has_el_number_count == _TRUE_){
          transfer_ic1_nc[index_d1] = fabs(transfer_ic1_nc[index_d1]);
          transfer_ic2_nc[index_d1] = fabs(transfer_ic2_nc[index_d1]);
        }
      }
    }
